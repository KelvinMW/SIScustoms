...
if (isActionAccessible($guid, $connection2, '/modules/User Admin/user_manage_add.php') == false) {
    ...
    $row = $form->addRow();
        ...
        $row = $form->addRow();
        $row->addLabel('preferredName', __('Second Name'))->description(__('First name as shown in ID documents.'));
    ...
    $availableRoles = array_reduce(
        $availableRoles, function ($carry, $item) use (&$currentUserRoles, &$staffRoles, &$studentRoles) {
...
*/
    ...
    $row = $form ->addHiddenValue('emailAlternate', '');
    $row = $form->addRow();
    ...
    $row = $form->addHiddenValue('languageSecond', 'N/A');
    $row = $form->addHiddenValue('languageThird', 'N/A');
    $row = $form->addHiddenValue('ethnicity', 'N/A');
    $row = $form->addHiddenValue('religion', 'N/A');
    $row = $form->addRow();
    ...
    // EMPLOYMENT
    $row = $form -> addHiddenValue('profession', 'N/A');
    $row = $form -> addHiddenValue('employer', 'N/A');
    $row = $form -> addHiddenValue('jobTitle', 'N/A');
    ...
    $row = $form->addRow();
        ...
        $row->addTextField('studentID')
            ...
            ->setValue(isset($username) ? $username : '');
    ...
    $row = $form->addRow();
        ...
        $row = $form -> addHiddenValue('vehicleRegistration', 'N/A');
        $row = $form ->addHiddenValue('lockerNumber', 'N/A');
    ...
    if (!empty($studentAgreementOptions)) {
        $options = array_map(
            function ($item) {
                return trim($item); 
            }, explode(',', $studentAgreementOptions)
        );
